...
def capture_ba(myid, caseNo, myaddr):
    ...
    if html.find(no_result) != -1:
        ...
        sleep(2)
        try:
            # driver.find_element_by_xpath('//*[@id="map_canvas"]/div/div/div[8]/div[1]/div/button[2]').click()
            driver.find_element_by_xpath('//*[@id="map_canvas"]/div/div/div[11]/div/div[2]/div/button[2]').click()
        except:
            # driver.find_element_by_xpath('//*[@id="map_canvas"]/div/div/div[7]/div/div/button[2]').click()
            # driver.find_element_by_xpath('//*[@id="map_canvas"]/div/div/div[11]/div/div[2]/div/button[2]').click()
            driver.find_element_by_xpath('//*[@id="map_canvas"]/div/div/div[11]/div/div/div/button[2]').click()
...
if __name__ == '__main__':
    ...
    sql = 'SELECT id, cCaseNo, cAddress FROM `case_map` WHERE cAddress LIKE "新北市%" AND cAddress LIKE "%地號%" AND cMapLabel = 4 AND cPath = "" ORDER BY id DESC LIMIT 1000;'
    # sql = 'SELECT id, cCaseNo, cAddress FROM `case_map` WHERE id = "182986";'
    conn = db.cursor()
    conn.execute(sql)
    db.close()
