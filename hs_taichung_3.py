...
# 郵遞區號查詢
def zip_query(city, distinct):
    db = MySQLdb.connect("61.56.209.149","coder", "AccuHit5008!!", "foreclosure", port=3308, charset='utf8')    

    sql = 'SELECT * FROM `tziparea` WHERE zCity = "' + city + '" AND zArea = "' + distinct + '";'
...
def capture_ba(myid, caseNo, myaddr):
    ...
    #選擇縣市區域
    # zip = zip_query(myaddr[0], myaddr[1])
    myaddr[0] = myaddr[0].replace(u'台中市', u'臺中市')
    s1 = Select(driver.find_element_by_id('ZON2'))
    s1.select_by_value(myaddr[0]+myaddr[1])
    # s1.select_by_value('402')
...
if __name__ == '__main__':
    ...
    sql = 'SELECT id, cCaseNo, cAddress FROM `case_map` WHERE cAddress LIKE "台中市%" AND cAddress NOT LIKE "%地號%" AND cMapLabel = 4 AND cPath = "" ORDER BY id DESC LIMIT 1000;'
    conn = db.cursor()
    conn.execute(sql)
    db.close()
